// Package pluginmgr implements installation and upgrade of k8slocalcli
// plugins listed in the curated plugins.yaml index hosted alongside the CLI.
//
// The index maps short plugin names (e.g. "dashboard") to GitHub repos and
// optional template overrides. Plugin authors who follow the canonical
// release layout — assets named
//
//	k8slocalcli-<name>-<version>-<os>-<arch>.tar.gz
//	k8slocalcli-<name>-<version>-SHA256SUMS
//
...
// DefaultIndexURL is where `k8slocalcli plugin` fetches the curated plugin
// catalog. Users can override via the K8SLOCALCLI_PLUGINS_INDEX environment
// variable — useful for testing or for running a private index.
const DefaultIndexURL = "https://raw.githubusercontent.com/rogerwesterbo/k8slocalcli/main/plugins.yaml"

// IndexEnvVar is the environment variable consulted before falling back to
// DefaultIndexURL.
const IndexEnvVar = "K8SLOCALCLI_PLUGINS_INDEX"

// resolveIndexURL honors K8SLOCALCLI_PLUGINS_INDEX if set and non-empty.
...
// Entry is one row in plugins.yaml. All override fields are optional; their
// zero values fall back to conventional templates derived from Name and Repo.
type Entry struct {
...
// FetchIndex downloads plugins.yaml from the resolved index URL and parses it.
...
// resolve fills in the template placeholders for name/repo/version/os/arch on
// the given template, applying default templates when the entry-level
// override is empty.
...
// InnerBinaryPath returns the path inside the archive where the plugin binary
// lives (relative to the archive root).
func (e *Entry) InnerBinaryPath(version, goos, goarch string) string {
	return e.resolve(
		e.ArchiveInnerPath,
		"k8slocalcli-{name}-{version}-{os}-{arch}/k8slocalcli-{name}",
		version, goos, goarch,
	)
}
...
// CosignIdentity returns the OIDC identity regex cosign should accept when
...
// BaseDownloadURL returns the prefix under which release assets live for the
// given tag — callers append the asset filename.
