...
import (
	...
	"github.com/rogerwesterbo/k8slocalcli/internal/release"
)
...
func newPluginInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Fetches the curated plugin index (plugins.yaml) from the
k8slocalcli repo, resolves <name> to a GitHub release, downloads and verifies
the release archive (SHA-256 always, cosign signature when available), and
installs the binary next to k8slocalcli itself (or into --prefix).`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !ok {
				return fmt.Errorf("plugin %q is not in the curated index (see `k8slocalcli plugin list --available`)", name)
			}
			...
			_, _ = fmt.Fprintf(cmd.OutOrStdout(),
				"✅ %s %s installed — try `k8slocalcli %s --help`\n", state.Name, state.Version, state.Name)
			return nil
		},
	}

	cmd.Flags().StringVar(&installPrefix, "prefix", "",
		"install directory (default: same directory as k8slocalcli)")
	cmd.Flags().BoolVar(&installSkipChecksum, "skip-checksum", false,
		"skip SHA-256 verification (not recommended)")
	cmd.Flags().BoolVar(&installSkipCosign, "skip-cosign", false,
...
func newPluginUpgradeCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long: `With no arguments, upgrades every plugin recorded in
~/.k8slocalcli/plugins. With one or more names, upgrades only those. Plugins
already on the latest release are skipped.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, state := range targets {
				...
				// upgradeOne distinguishes no-op vs upgraded via its own
				// output; to decide the summary we re-read state and compare.
...
// resolveUpgradeTargets returns the set of plugin states to upgrade. An empty
// names slice expands to all installed plugins.
...
// upgradeOne checks the plugin's repo for a newer release and, if available,
// reinstalls it. No-op when already current.
func upgradeOne(ctx context.Context, stdout, stderr io.Writer, idx *pluginmgr.Index, state *pluginmgr.State) error {
	...
	if !ok {
		// Entry vanished from the index — still try to upgrade from the repo
		// recorded in state by synthesising a minimal entry.
...
// dirOf returns the directory component of p. Used to keep plugin upgrades
// installing back to their original location.
